...
while IFS= read -d $'\0' -r dir; do
    ...
    (
        cd "${builddir}"
        log "Building: ${image}"
        docker build -t "${image}" --build-arg APP_NAME=$svcname --build-arg BRANCH_NAME=${BRANCH} --build-arg BUILD_NAME=${BUILD_NUMBER} --build-arg SEALIGHTS_TOKEN=${SEALIGHTS_TOKEN} .
    )
